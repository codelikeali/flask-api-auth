...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://phpmyadmin:Root!123@127.0.0.1/flask_api'
...
class Users(db.Model):
  ...
  authors = db.relationship('Authors',backref='creater')
...
def get_all_users(): 
   ...
   for user in users:   
      ...
      #  user_data['books'] = jsonify({smallItem.name for smallItem in user.authors})  
...
# Authors Releated Work


@app.route('/getCurrentAuthorBooks', methods=['GET', 'POST']) 
@token_required
def getCurrentAuthorBooks(current_user):
   authorBooks = Authors.query.filter_by(id=current_user.id).all()
   output = []  
   ...
   return jsonify({'currentAuthorBooks' : output})
...
def create_author(current_user):
   ...
   new_authors = Authors(name=data['name'], country=data['country'], book=data['book'], booker_prize=True, creater_id=current_user.id)  
